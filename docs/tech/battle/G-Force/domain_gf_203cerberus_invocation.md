...
| Counter (tick) | — | `0xB0C82A` | `++*(WORD*)(gfIfrit_sequenceCtxPtr + 50)` |
...
### Shared GF Infrastructure (gfIfrit_ prefix — shared across all FamilyB GFs)

| Address | Current Name | Type | Description |
|---------|-------------|------|-------------|
| `0x27973EC` | `gfIfrit_sequenceCtxPtr` | dword | Active GF sequence context (resolved per-tick) |
| `0x27973B8` | `gfIfrit_runtimeSlotPtr` | dword | Active GF runtime slot (resolved per-tick) |
| `0x27973BC` | `gfIfrit_renderCtxPtr` | dword | Active GF render context (resolved per-tick) |
| `0x27973C0` | `gfIfrit_sequenceStatePtr` | dword | Active GF state pointer (resolved per-tick) |
| `0x2797624` | `isGF_SequenceOffsetStack` | dword | Active GF stack frame (resolved per-tick) |

**Note:** The `gfIfrit_` globals are shared across all GFs. Only one GF cinematic runs at a time. `GF_203Cerberus_ResolveSharedCtx` resolves these pointers from Cerberus's configuration tables at `0x1873170`-`0x1873180` on every tick.
...
STAGE 2: GF Cinematic State Machine
  ...
  → State 3: Calls g_GfActiveCallbackPtr = GF_203Cerberus_InvokeSummonScript
...
- The `gfIfrit_*` shared globals are poorly named — they are shared across ALL GFs, not specific to Ifrit. Only one GF cinematic runs at a time, so the same globals are reused. Renaming them to `GF_SharedCinematic_*` is deferred to avoid disrupting other GF analyses.
