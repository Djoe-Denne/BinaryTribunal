# MAG_223_METEOR_SequenceTick @ 0xA8FF00

- Instr (live): 120
- Palier: high
- GLM A: fence_found=true finish_reason=stop reasoning_tokens=381
- GLM B: fence_found=true finish_reason=stop reasoning_tokens=237
- GLM C: fence_found=true finish_reason=stop reasoning_tokens=244
- A==B: non
- Push IDB: oui
- SetType: unsigned int __cdecl MAG_223_METEOR_SequenceTick(void)
- Notes parent: Tick FamilyB Meteor. WORD ++ [ctx+32h], BYTE parity slot+41h puis copie +40h. Bit 400h WORD [ctx+0] -> OT+0Ch / WORD render+24h. Pause BYTE +35h depuis flag bataille bit0. Miroir camera 1D97778..94 vers deux banques (1D97788 & FFFFh). Interp signe [88h]=[84h]+[8Ch]*movsx slot+40h. Deux tests BYTE [ctx+1]&80h. 3 passes si +35h==0. BS_Debug arg = DWORD [slot+38h] (OT+44h, pas occupancy). Ret ((~WORD state+0Ah)>>14)&2. Occupancy/0xD0/0x1D0/0x44 GF/K_GF 0x84/stride 0x9C absents.
 ...
 * 120 instr, size 0x215, end 0xA90115. IDA type unsigned int(). cdecl, 0 args, retn C3.
 * Labels loc_A8FF5B / loc_A8FF6A / loc_A8FF6D / loc_A9008C / loc_A9009D / loc_A900C4.
 ...
 * add edx,44h = g_BattleOTBase+0x44 -> slot+38h, not GF Exists. No domain::.
 */

int au_re_bs_modulo_41(void);
...
unsigned int __cdecl MAG_223_METEOR_SequenceTick(void)
{
  unsigned int seqCtx;
  unsigned int slot;
  unsigned int render;
  ...
  *(_DWORD *)(seqCtx + 0x88) = *(_DWORD *)(seqCtx + 0x84) + interp;
  ...
  {
    slot = g_GfCinematic_RuntimeSlotPtr;
    BS_Debug_UnknownFloatOperations(*(_DWORD **)(slot + 0x38), 4096);
  }
  seqCtx = g_GfCinematic_SequenceCtxPtr;
  if ( *(_BYTE *)(seqCtx + 1) & 0x80 )
    sub_A90120();
