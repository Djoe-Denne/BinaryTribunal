# GF_206Eden_SequenceTick @ 0xAE3470

- Instr (live): 120
- Palier: high
- GLM A: fence_found=true finish_reason=stop reasoning_tokens=1413
- GLM B: fence_found=true finish_reason=stop reasoning_tokens=1835
- GLM C: fence_found=true finish_reason=stop reasoning_tokens=1482
- A==B: non
- Push IDB: oui
- SetType: unsigned int __cdecl GF_206Eden_SequenceTick(void)
- Notes parent: FamilyB Eden. inc WORD [seq+0x32], BYTE slot+41h=counter&1 puis copie +40h. Dual copy 8 DWORD (mask 0xFFFF seulement 1D97788). OT+0x44 → slot+38h (pas GFSG). Interp signed byte*seq+8Ch + seq+84h → +88h (pas K_GF). Deux if [seq+1]&80h. EAX=((~WORD[state+0Ah])>>14)&2. Occupancy 1+2 absente. add esp 8 puis 4.
 ...
 * 120 instr, size 0x215, end 0xAE3685. IDA type unsigned int(). cdecl, 0 args, retn C3. FLAGS 0x5400. FRSIZE 0.
 * push ebx/esi/edi at 0xAE34E2 (after early BYTE/WORD stores). No sub esp. No arg_0.
 * add esp,8 after BS_Debug_UnknownFloatOperations; add esp,4 after Call_Bs_parseCamera2.
 * _rand EAX discarded. Dual 8-DWORD copy; 0xFFFF only on dword_1D97788.
 ...
 * Labels loc_AE34CB / loc_AE34DA / loc_AE34DD / loc_AE35FC / loc_AE360D / loc_AE3634.
 */

int __cdecl au_re_bs_modulo_48(void);
int __cdecl _rand(void);
unsigned int *__cdecl BS_Debug_UnknownFloatOperations(unsigned int *ot, int a2);
int __cdecl sub_AE3690(void);
int __cdecl sub_AF3F20(void);
int __cdecl sub_AE3790(void);
int __cdecl sub_AF4340(void);
int __cdecl sub_AEEC40(void);
int __cdecl Call_Bs_parseCamera2(int);
int __cdecl sub_AE2F10(void);
...
unsigned int __cdecl GF_206Eden_SequenceTick(void)
{
    unsigned char *seq;
    unsigned char *slot;
