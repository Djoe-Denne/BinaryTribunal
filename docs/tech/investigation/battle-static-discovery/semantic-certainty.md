...
| Classe | Nombre | Règle |
|---|---:|---|
| CERTAIN | 22 (8 doc + 3 triple Grok + 7 GPU 1+P + 2 GPU 2+1 + 2 GPU 1+V) | name_only_known+A==B+push, wrapper trivial, ou R/1+P=CERTAIN poussé |
| LIKELY | 286 | decomp poussé (wiki ± A==B) ; +Gfx internes Square dès que decomp |
...
File Grok (LIKELY+UNCERTAIN+CONFLICT) : **294**, dont filtre bataille `0x47xxxx`–`0x51Bxxx` : **218**. Gfx internes avec decomp (`0x40763D`, `0x4980C0`, `0x499EA0`) : **après** le lot battle en cours, ordre d'adresse.
...
- **File Grok maintenant** (ont un `decomp/`) : `0x40763D`, `0x4980C0` `Gfx_SubmitDisplayLists`, `0x499EA0` `Gfx_SubmitViewportLists`.
