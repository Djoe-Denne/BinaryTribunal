...
- `cameraStruct` (`0x1D977A8`) is a two-slot pool of active camera animation states; `cameraStructPointer` (`0x1D97798`) points at the currently active slot chosen by `BattleCamera_StartTrack`.
- `cameraRelated_pointerAnimColl` (`0x1D97718`) is the main camera-animation control word. `BattleCamera_StartTrack` sets active-variant bits here, `BS_CameraAnim_Tick` clears them when a camera script finishes, and the generic/GF selectors also use high bits as force/hold flags. These bits do not select the two physical pool records, but each record stores its owning variant in byte 0 for release.
...
- `word_1D8E038` and `word_1D977A2` are camera-side scalar outputs advanced by `BS_CameraAnim_Tick`; the code treats them like camera/FOV-like parameters, but the exact semantic label is still not proven.^[ambiguous]
...
2. `BS_CameraRelated_battle_reset` clears battle camera flags, calls `InitCameraStruct`, runs `BattlePresentation_InitBuffersAndProjection`, initializes battle presentation task lists, and derives a camera-movement speed from `SG_CAMERA_MOVEMENT_SETTING`.
3. Stage-specific setup then calls `BS_CameraInit` (`0x500F70`), which selects one of the encounter's `main_camera` entries from `CURRENT_ENCOUNTER_DATA_SCENE_OUT` and binds the stage camera tables via `BattleCamera_BindResourceSections`.
4. During the active render/presentation loop, `BdLink_GF_battle_input_and_texture_upload` calls `updateBattleCamera` and `BattleCamera_BuildViewAndConsumeDeltas` every frame before the final `Call_Bs_parseCamera2` / `Call_Bs_ParseCamera` projection work.
...
| --- | --- | --- | --- |
| Generic action presentation | `BattleTaskQueue_Tick` -> `BattleTaskQueue_Dispatch('h')` -> `BattleActionSequence_DispatchTick` default path -> `BattleActionSequence_Tick_Generic` | `BattleActionSequence_Tick_Generic` state 2 -> `BattleActionSequence_SelectGenericCameraAnimation` | Chooses a stage camera animation from attacker/target state and queues it through `BattleCamera_StartTrack`, then loads the effect callback through `BattleGF_LoadCallbackByMagicID`. |
...
- `BS_CameraAnim_Tick` (`0x5035E0`) is the active camera-script runner. It advances the current camera keyframe program, interpolates world and look-at triplets, updates the extra camera scalars, and clears the active variant bit in `cameraRelated_pointerAnimColl` when the script finishes.
- `BattleCamera_BuildViewAndConsumeDeltas` (`0x5033E0`) rebuilds derived camera orientation/motion from the current world/look-at values and applies accumulated deltas.
...
- `BattleCamera_BindResource` (`0x5099A0`) is a direct effect-script hook that only binds a camera resource through `BattleCamera_BindResourceSections`; it does not itself start a track. Multiple `MAG_*` entries call it, so those effects provide their own camera tables.

## Domain Boundary

- The domain/presentation split from [[projects/re-ff8/concepts/draw-magic-and-render-bridge]] still holds for camera ownership, but not every `BattleActionSequence_Tick_*` is side-effect-free. The physical-with-events path reaches `0x506690 → 0x493D80` before its popup and can apply authoritative result records.
...
- The presentation-critical contract is the camera/effect side: `BS_CameraInit`, `BattleActionSequence_SelectGenericCameraAnimation`, `BattleGF_LoadCallbackByMagicID`, `updateBattleCamera`, `BS_CameraAnim_Tick`, and any `MAG_*` callback that calls `BattleCamera_BindResource` directly.

For an external presentation replacement, preserving those presentation-critical hooks keeps the original camera semantics. Result application reached from a sequence worker must nevertheless remain in the domain adapter rather than being dropped as “cosmetic”.^[inferred]

## IDA Updates Applied

- Renamed `0x56CD50` -> `CameraBasis_CopyDefaultAndApplyEulerS16`
...
Later consolidation also proved that the pool has two matching BdLink nodes,
that each `0x524` record has capacity for 32 keyframes, and that
`BattleCamera_ReturnBlendTick` returns `0` during the ramp and `2` only at its
deadline. Wave3 (2026-09-10) further fixed the `0x50633D` writer to constant
`0x8000`, the 58 embedded setters to one cloned MAG opcode, bits 0–6 to a
7-actor mask, and the 32-keyframe bound to `float v31[32]` in `sub_50D060`.
See the canonical
[[projects/re-ff8/concepts/battle-camera-architecture]] page.
...
- Confirm `BYTE2` (`0x1D9771A`) live: write watchpoint, break if the written value ∈ `{2,3}` (static writer unique `=1` at `InitCameraStruct` `0x50421F`; `{2,3}` have no `.text` writer).
