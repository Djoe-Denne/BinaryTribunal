...
summary: Gap analysis for a faithful (ISO) reimplementation of the FF8 battle loop. Separates gaps into (A) recoverable now from the well-annotated IDB but not yet distilled, (B) structural gaps not previously flagged, and (C) genuinely runtime-pending items — with an ISO-blocking triage and a closure order.
provenance:
  extracted: 0.86
  inferred: 0.11
  ambiguous: 0.03
...
### A4. ATB internals — *CLOSED 2026-06-14*
`BattleATB_TickAndReady` (`0x4842B0`) decompiled: called once per frame from `BattleUI_HudInputAndATBTick` (`0x4A84E0`) **only when `!IS_BATTLE_PAUSED`**; iterates slots **ascending 0→N**; two passes (GF summon-charge timers `2/3/1` by Haste/Slow, then per-slot ATB with the documented formula + readiness routing); polls escape at tail. Distilled into [[projects/re-ff8/concepts/atb-and-command-menu]] and [[projects/re-ff8/concepts/battle-lifecycle]].
