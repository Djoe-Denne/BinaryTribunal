...
- G22 v18 protocol-v4 ordinary `p1-g22-v18-ordinary-visible-2026-09-02.json` and shutdown `p1-g22-v18-shutdown-2026-09-02.json` are collector `FAIL` / `Faulted` with `negative_runtime_evidence=['runtime entered Faulted']`. The host produced enemy-slot `eligible_mask=0x08`; protocol v4 refused that non-zero mask with zero enqueue writes. Shutdown restored hook bytes exactly (`0x07748cc4`) but the logical state stayed `Faulted`. Not promotion evidence. Report `g22-protocol-v4-live-waiver-2026-09-01.md`.
